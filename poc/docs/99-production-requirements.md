...
| 4 | QUIC mesh 전송 | fd 제한으로 수평 확장 불가 | **P0** |
| 5 | Graceful shutdown | 진행 중 요청 끊김 + peer 에러 감지 지연 | P1 |
| 6 | Observability | 장애 원인 파악 불가 | P1 |
| 7 | Peer 간 인증 | 아무나 mesh에 참여 가능 | P1 |
| 8 | Config reload | 설정 변경시 재시작 필요 | P2 |
...
```
릴레이 요청마다 새 TCP 연결 생성
1000 동시 릴레이 = 2000 fd → ulimit에 의한 상한선
drps를 늘려도 fd 제한이 병목
```

### 필요한 것

mesh 전송 계층(drps ↔ drps)을 QUIC으로 전환:

| 항목 | 현재 (TCP) | 변경 (QUIC) |
|------|-----------|-------------|
| fd/peer | 릴레이 수에 비례 | 1 (UDP 소켓) |
| 릴레이 생성 | TCP 3-way handshake (1.5 RTT) | stream open (0 RTT) |
| HoL blocking | 있음 | 없음 |
| 암호화 | 별도 설정 | TLS 1.3 내장 |

### 왜 필요한가

- fd 제한 해소: drps 추가 시 진정한 수평 확장 가능
- peer 인증 (#7)이 mTLS로 자연스럽게 해결
- heartbeat (#2)가 QUIC KeepAlive로 대체 가능

### 설계 영향

- drpc ↔ drps는 TCP 유지 (인터넷 경유, UDP 차단 가능성)
- Go 구현 시 `quic-go` 라이브러리 사용
- 상세: [ADR-005](../../docs/adr/005-mesh-transport-quic.md)
...
relay ✓                         + QUIC mesh 전송 (ADR-005)
drpc disconnect ✓               + graceful shutdown
                                + observability
                                + peer 인증 (QUIC mTLS로 해결)
                                + config reload
```
